...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.2.0-34D399?style=for-the-badge" alt="version" />
</p>
...
в шаблонах и тексте обслуживания можно писать плейсхолдер `{bot}`.

Интерфейс — русский. Выплаты пользователям — **только Telegram Stars**. CryptoBot нет.

Автор: [KodoDrive](https://github.com/svod011929) · Релиз: [v1.2.0](https://github.com/svod011929/KodoStars/releases/tag/v1.2.0)
...
- Активация реферала: активность + проверка устройства + минимум **`REFERRAL_MIN_PIARFLOW_SUBS`** (по умолчанию 2) ресурсов PiarFlow со статусом `subscribed` (награда засчитана в интеграции)
...
- **Амбассадор**: заявка на канал/чат/бот → после одобрения кастомные реф-условия и дневной промокод (опциональный автопост в канал/чат)
...
| Настройки | экономика (в т.ч. `referral_min_piarflow_subs`), антитвинк и операционные флаги — **без перезапуска** (БД, валидация как `.env`) |
| PiarFlow | тумблер провайдера в рантайме (`provider_states`) |
| Админы | владельцы из `ADMIN_IDS` добавляют/удаляют админов прямо в боте |
| Журнал / Антифрод | аудит всех админ-действий, лента фрод-событий, кластеры твинков |
...
ежедневка и серии, задания, рефералы (первый старт, уровни, доля, гейт по paid PiarFlow),
промокоды, настройки в рантайме, роли админов, аудит, фоновая рассылка, статистика/экспорт,
миграции, контракт PiarFlow, Fragment-конфиг, premium emoji helpers, middleware и
**end-to-end** через диспетчер с фейковой Telegram-сессией (`tests/fake_telegram.py`).
...
app/
  ...
  bot/
    ...
    handlers/            # start, commands, cabinet, earn, withdraw, promo, payments, system
    admin/               # панель: router, texts, keyboards, states + разделы
  ...
  services/              # ledger, referrals, daily, tasks, boosts, payments, withdrawals,
                         ...
                         # fragment, piarflow_webhook, piarflow_quality, channels, …
