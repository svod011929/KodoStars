...
Монетизация трафика — обязательная подписка (**PiarFlow + Tgrass**): при пройденном
антитвинке сначала PiarFlow, затем Tgrass; без проверки — только Tgrass. Вебхуки отписок
со штрафом, реферальный бонус только после ≥N оплаченных подписок PiarFlow.
Показы рекламы — **BotoHub Views**. Выплата Stars — через **Fragment** (mnemonic + cookies);
завершённые выплаты можно логировать в канал (`PAYOUT_LOG_CHAT_ID`).
...
трафик / система). Настройки админки — тоже по группам.
...
- Кнопка **«Поделиться ссылкой»** (`t.me/share/url`) называет выплату за друга; на экране рефералов видно место в топе
- После входа может прийти **приветка** из ротации (текст и кнопка задаёт админ; на стене ОП её нет)
...
Корень админки — компактный хаб: операции · амбассадоры · каталог · ОП · система · настройки.
...
Каскад обязательной подписки зависит от антитвинка:
...
Кампания закупки: `https://t.me/<bot>?start=c_<CODE>` (или `utm_<CODE>`).
Каждый заход — клик, уникальные id — пользователи. Цифры день/неделя/всё время
